...
public class BoardRepositoryImpl implements BoardRepository {
	...
	public void getPerPageBorardList(int page, Model model) {
		...
		String SQL = "SELECT * FROM board ORDER BY b_num DESC LIMIT ?, ?"; // 각 페이지에서 게시글을 index 0번부터 10개씩 출력. 아래에 있는
																			// query의 파라미터에 변수 값을 넣음으로써 작동
	...
	public String getPostDateOfBoard(String b_date, LocalDate now) {
		...
		if (ChronoUnit.DAYS.between(targetDate, now) > 0) {
			result = targetDateAll.format(DateTimeFormatter.ofPattern("yyyy.MM.dd")); // 현재시간과 작성일을 계산해서 0보다 크면(다음 날이면)
																						// 날짜를 담음
		} else {
	...
	public List<Map<String, Object>> getBoardList(Criteria cri) throws Exception {
		return template.queryForList("SELECT * FROM board ORDER BY b_num DESC LIMIT ?, ?", cri.getPageStart(),
				cri.getPerPageNum());
	}
	...
	public void searchBoardDetail(String num, Model model) {
		...
		for (Board board : BoardList) {
			if (board.getB_num() == numInt) { // DAO 객체 안의 B_num과 model 안의 b_num이 일치하면 실행
				boardInfo = board;
				increaseViews(num);
				break; // 이게 있어야 일치하는 값을 찾았을 때 실행하고 바로 빠져나온다.
			} 
	...
	// 게시판 상세보기에서 이전글, 다음글 버튼 기능. searchBoardDetail 메서드 안에서 호출된다.
